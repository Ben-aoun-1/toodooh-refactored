...
import { Calendar, ChevronLeft, ChevronRight, Crosshair, DollarSign, Grid3X3, List, Loader2, MapPin, Monitor, MoreVertical, PartyPopper, Search, TrendingUp, X } from 'lucide-react';
...
type OwnerCampaignStatusFilter = 'all' | 'active' | 'upcoming' | 'pending' | 'completed' | 'rejected';
...
const getStatusUi = (status: string, isUpcoming: boolean) => {
  if (isUpcoming) return { label: 'A venir', badge: 'bg-blue-50 text-blue-700 border border-blue-200', dot: 'bg-blue-500' };
  const key = (status || '').toLowerCase();
  if (key === 'active') return { label: 'Active', badge: 'bg-green-50 text-green-700 border border-green-200', dot: 'bg-green-500' };
  if (key === 'pending') return { label: 'En attente', badge: 'bg-orange-50 text-orange-700 border border-orange-200', dot: 'bg-orange-500' };
  if (key === 'completed') return { label: 'Terminée', badge: 'bg-gray-100 text-gray-700 border border-gray-200', dot: 'bg-gray-500' };
  if (key === 'rejected') return { label: 'Refusée', badge: 'bg-red-50 text-red-700 border border-red-200', dot: 'bg-red-500' };
  if (key === 'paused') return { label: 'En pause', badge: 'bg-gray-100 text-gray-700 border border-gray-200', dot: 'bg-gray-500' };
  if (key === 'draft') return { label: 'Brouillon', badge: 'bg-amber-50 text-amber-700 border border-amber-200', dot: 'bg-amber-500' };
  return { label: 'En attente', badge: 'bg-orange-50 text-orange-700 border border-orange-200', dot: 'bg-orange-500' };
};
...
export default function OwnerCampaigns() {
  ...
  useEffect(() => {
    const loadOwnerCampaigns = async () => {
      ...
      try {
        ...
        const [{ data: ownerLocations, error: ownerLocationsError }, { data: ownerScreens, error: ownerScreensError }, { data: ownerApprovals, error: ownerApprovalsError }] =
          await Promise.all([
            supabase.from('locations').select('id').eq('owner_id', user.id),
            supabase.from('screens').select('id, location_id').eq('owner_id', user.id).eq('status', 'active'),
            supabase.from('campaign_owner_approvals').select('campaign_id, status').eq('owner_id', user.id),
          ]);
        ...
        const ownerScreenToLocation = new Map<string, string | null>(
          (ownerScreens || []).map((s: { id: string; location_id: string | null }) => [s.id, s.location_id || null])
        );

        const queryTasks: Promise<any>[] = [
          ownerLocationIds.length > 0
            ? supabase.from('campaign_locations').select('campaign_id, location_id').in('location_id', ownerLocationIds)
            : Promise.resolve({ data: [], error: null }),
          ownerScreenIds.length > 0
            ? supabase.from('campaign_screens').select('campaign_id, screen_id').in('screen_id', ownerScreenIds)
            : Promise.resolve({ data: [], error: null }),
        ];
        ...
        const [{ data: allCampaignLocRows, error: allCampaignLocError }, { data: allCampaignScreenRows, error: allCampaignScreenError }] =
          await Promise.all([
            supabase.from('campaign_locations').select('campaign_id, location_id').in('campaign_id', campaignIds),
            supabase.from('campaign_screens').select('campaign_id, screen_id').in('campaign_id', campaignIds),
          ]);
        ...
        (ownerApprovals || []).forEach((approval: { campaign_id: string; status?: 'pending' | 'approved' | 'rejected' }) => {
        ...
        if (advertiserUserIds.length > 0) {
          ...
          (advertiserProfiles || []).forEach((profile: { user_id: string; business_name?: string | null; logo_url?: string | null }) => {
            if (profile.logo_url) advertiserLogoByUserId.set(profile.user_id, profile.logo_url);
            if (profile.logo_url && profile.business_name) {
              advertiserLogoByBusinessName.set(profile.business_name.trim().toLowerCase(), profile.logo_url);
            }
          });
        }
        if (advertiserClientNames.length > 0) {
          ...
          if (advertiserProfilesByNameError) {
            console.warn('OwnerCampaigns logo query by business_name failed:', advertiserProfilesByNameError);
          }
          (advertiserProfilesByName || []).forEach((profile: { business_name?: string | null; logo_url?: string | null }) => {
            if (profile.logo_url && profile.business_name) {
              advertiserLogoByBusinessName.set(profile.business_name.trim().toLowerCase(), profile.logo_url);
            }
          });
        ...
        for (const row of allCampaignLocRows || []) {
          totalLocationCountByCampaign.set(row.campaign_id, (totalLocationCountByCampaign.get(row.campaign_id) || 0) + 1);
        }
        const totalScreenCountByCampaign = new Map<string, number>();
        for (const row of allCampaignScreenRows || []) {
          totalScreenCountByCampaign.set(row.campaign_id, (totalScreenCountByCampaign.get(row.campaign_id) || 0) + 1);
        }

        const cards: OwnerCampaignCard[] = (campaignsRows || [])
          .filter((campaign: any) => {
            const advertiserValidated = campaign?.content_validation_status === 'approved';
            const videoValidated = Boolean(campaign?.video_id && approvedVideoIdSet.has(campaign.video_id));
            return advertiserValidated && videoValidated;
          })
          .map((campaign: any) => {
          const ownerLocIds = ownerLocationIdsByCampaign.get(campaign.id) || new Set<string>();
          const ownerScreenIdsForCampaign = ownerScreenIdsByCampaign.get(campaign.id) || new Set<string>();
          ...
          const ownerShare = Math.min(1, Math.max(0, totalLocationCount > 0 ? locationShare : screenShare || 1));
          ...
          const baselineImpressions = plannedImpressions > 0
            ? plannedImpressions
            : (campaignViews > 0 ? campaignViews : Math.round((campaignBudget / cpmTnd) * 1000));
          ...
          return {
            ...
            clientLogoUrl:
              ...
              advertiserLogoByBusinessName.get(String(campaign.client?.name || '').trim().toLowerCase()) ||
              null,
          };
          });
  ...
  const filteredCampaigns = useMemo(() => {
    ...
    return campaigns.filter((campaign) => {
      ...
      const matchesStatus =
        statusFilter === 'all' ||
        (statusFilter === 'upcoming' ? isUpcoming : (campaign.status || '').toLowerCase() === statusFilter);
  ...
  const handleApproveSelectedCampaign = async () => {
    ...
    try {
      ...
      await campaignOwnerApprovalService.approveCampaign(
        selectedCampaign.id,
        user.id
      );
      setCampaigns((prev) =>
  ...
  const handleRejectSelectedCampaign = async () => {
    ...
    try {
      ...
      await campaignOwnerApprovalService.rejectCampaign(
        selectedCampaign.id,
        user.id,
        undefined
      );
  ...
  return (
    <div className="min-h-screen bg-white">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6">
                  {[
                    ...
                    >
                      ...
                      <p className={`text-[34px] leading-9 font-medium mt-1 tabular-nums ${stat.color}`}>
                        {stat.value}
                      </p>
                    </div>
                  ))}
              ...
              <div className="flex flex-col lg:flex-row lg:items-center gap-3 mb-5">
                <div className="flex flex-wrap items-center gap-2">
                  {[
                    ...
                    { key: 'completed' as const, label: 'Terminées', count: statusCounts.completed },
                ...
                {viewMode === 'grid' ? (
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start justify-between gap-3 mb-2 min-h-[28px]">
                            <h3 className="text-base font-semibold text-[#171717] truncate pr-2">{campaign.name}</h3>
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${statusUi.badge}`}>
                          ...
                          <div className="grid grid-cols-2 gap-6 mb-4 w-full">
                            <div>
                              ...
                              <p className="text-base font-semibold text-[#171717]">
                                {campaign.estimatedRevenue.toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} TND
                              </p>
                            </div>
                  ...
                  <div className="rounded-xl border border-[#EBEBEB] bg-white overflow-hidden">
                    <div className="grid grid-cols-[1.4fr_1fr_1fr_1fr_1fr_1fr_44px] items-center bg-[#F8F8F8] px-4 py-3 text-[13px] text-[#5C5C5C]">
                      ...
                      <div className="inline-flex items-center gap-1 font-medium"><Calendar className="h-3.5 w-3.5" />Date de debut</div>
                      <div className="inline-flex items-center gap-1 font-medium"><Calendar className="h-3.5 w-3.5" />Date de fin</div>
                      <div className="inline-flex items-center gap-1 font-medium"><DollarSign className="h-3.5 w-3.5" />Revenu</div>
                      <div className="inline-flex items-center gap-1 font-medium"><TrendingUp className="h-3.5 w-3.5" />Impressions</div>
                      <div />
                    </div>
                    ...
                    {paginatedCampaigns.map((campaign) => {
                      ...
                      const statusUi = getStatusUi(campaign.status, isUpcoming);
                      return (
                        <div key={campaign.id} className="grid grid-cols-[1.4fr_1fr_1fr_1fr_1fr_1fr_44px] items-center px-4 py-4 border-t border-[#F0F0F0] text-sm text-[#1F1F1F]">
                          ...
                          <div>{campaign.startDate ? campaign.startDate.toLocaleDateString('fr-FR') : '—'}</div>
                          <div>{campaign.endDate ? campaign.endDate.toLocaleDateString('fr-FR') : '—'}</div>
                          <div>{campaign.estimatedRevenue.toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} TND</div>
                          <div>{campaign.impressions.toLocaleString('fr-FR')}</div>
                          <div className="relative">
                            <button
                              type="button"
                              onClick={() => setOpenActionMenuId((prev) => (prev === campaign.id ? null : campaign.id))}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <p className="text-sm text-gray-700">
                        Affichage de <span className="font-medium text-[#00263A]">{filteredCampaigns.length === 0 ? 0 : startIndex + 1}</span> a{" "}
                        <span className="font-medium text-[#00263A]">
                          {Math.min(startIndex + itemsPerPage, filteredCampaigns.length)}
                        </span>{" "}
                        sur <span className="font-medium text-[#00263A]">{filteredCampaigns.length}</span> resultats
                      </p>
                    </div>
      ...
      {showDetailsModal && selectedCampaign && (() => {
        const statusUi = getStatusUi(selectedCampaign.status, Boolean(selectedCampaign.startDate && selectedCampaign.startDate > new Date()));
        const startStr = selectedCampaign.startDate ? selectedCampaign.startDate.toLocaleDateString('fr-FR') : '—';
        const endStr = selectedCampaign.endDate ? selectedCampaign.endDate.toLocaleDateString('fr-FR') : '—';
        const durationDays =
          selectedCampaign.startDate && selectedCampaign.endDate
            ? Math.max(0, Math.ceil((selectedCampaign.endDate.getTime() - selectedCampaign.startDate.getTime()) / (1000 * 60 * 60 * 24)))
            : 0;
        return (
          <div className="fixed inset-0 z-50 overflow-hidden">
            ...
            <div
              ...
              style={{ boxShadow: '0px 16px 32px rgba(14,18,27,0.102)', border: '1px solid #EBEBEB', borderRadius: '12px' }}
            >
              <div className="flex-none flex flex-row items-start p-4 gap-3 border-b border-[#EBEBEB]">
                <div className="flex flex-col gap-1 min-w-0 flex-1">
                  <h3 className="text-[20px] leading-6 font-semibold text-[#171717] truncate">{selectedCampaign.name}</h3>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium w-fit ${statusUi.badge}`}>
                    <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${statusUi.dot}`} />
                    {statusUi.label}
                  </span>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <div className="border-b border-[#EFEFEF] pb-3">
                  ...
                  <div className="flex justify-between text-sm text-[#171717]">
                    <span><strong>Début:</strong> {startStr}</span>
                    <span><strong>Fin:</strong> {endStr}</span>
                    <span><strong>Durée:</strong> {durationDays} jours</span>
                  </div>
                </div>
                ...
                <div className="border-b border-[#EFEFEF] pb-3">
                  ...
                  <div className="flex justify-between text-sm text-[#171717]">
                    <span><strong>Nombre de zones:</strong> {Math.max(1, selectedCampaign.ownerLocationsCount)}</span>
                    <span><strong>Zone couverte:</strong> —</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="rounded-xl border border-[#EBEBEB] overflow-hidden bg-black/5">
                    {campaignVideo?.url ? (
                      <video src={campaignVideo.url || undefined} controls className="w-full aspect-video object-contain bg-black" />
                    ) : (
                      <div className="aspect-video flex items-center justify-center text-sm text-[#A3A3A3]">Aucun spot</div>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="flex-none p-4 border-t border-[#EBEBEB]">
                {isPendingForOwner(selectedCampaign) ? (
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      {processingDecision === 'accept' && <Loader2 className="h-4 w-4 animate-spin" />}
      ...
      {showApprovalSuccessModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40" onClick={() => setShowApprovalSuccessModal(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-[760px] p-8 text-center relative" onClick={(e) => e.stopPropagation()}>
            ...
            <h3 className="text-[42px] leading-none font-semibold text-[#171717] mb-5">Félicitations !</h3>
