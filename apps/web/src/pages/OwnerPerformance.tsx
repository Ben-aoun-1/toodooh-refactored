...
import { ArrowDown, ArrowUp, Calendar, Clock3, Eye, MapPin, Monitor, Users, Wallet } from 'lucide-react';
...
const getPresetRange = (preset: PerformancePeriodPreset) => {
  ...
  if (preset === 'year') return { startDate: toIsoDate(new Date(today.getFullYear(), 0, 1)), endDate: toIsoDate(today) };
  if (preset === 'quarter') {
...
function KpiCard({ title, value, diff, icon }: { title: string; value: string; diff: number; icon: React.ReactNode }) {
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-4">
      ...
      <div className="mt-3 flex items-center gap-1">
        {positive ? <ArrowUp className="h-4 w-4 text-emerald-500" /> : <ArrowDown className="h-4 w-4 text-rose-500" />}
...
export default function OwnerPerformance() {
  ...
  useEffect(() => {
    const loadOwnerScope = async () => {
      if (!user?.id) return;
      const [{ data: ownerLocations }, { data: ownerScreens }, { data: ownerApprovals }] = await Promise.all([
      ...
      const ids = Array.from(
        new Set([
          ...((ownerCampaignLocRes.data || []) as Array<{ campaign_id: string }>).map((r) => r.campaign_id),
          ...((ownerCampaignScreenRes.data || []) as Array<{ campaign_id: string }>).map((r) => r.campaign_id),
  ...
  const loadData = async (nextFilters: PerformanceFilters) => {
    try {
      setLoading(true);
      setError('');
      const data = await performanceService.getDataset(nextFilters, { campaignIds: ownerCampaignIds });
      setDataset(data);
  ...
  const onFilterChange = (key: keyof PerformanceFilters, value: string) => setFilters((prev) => ({ ...prev, [key]: value }));
  const applyPreset = (preset: PerformancePeriodPreset) => {
    if (preset === 'month' || preset === 'quarter' || preset === 'year') {
      const range = getPresetRange(preset);
      setFilters((prev) => ({ ...prev, preset, startDate: range.startDate, endDate: range.endDate }));
      return;
    }
  ...
  const kpiValues = useMemo(() => {
    const current: PerformanceKpis = dataset?.kpis || { diffusionSeconds: 0, impressions: 0, affluence: 0, activeScreens: 0, spend: 0 };
    const previous = dataset?.previousKpis || current;
    return [
      { title: 'Durée de diffusion', value: formatDuration(current.diffusionSeconds), diff: percentageDiff(current.diffusionSeconds, previous.diffusionSeconds), icon: <Clock3 className="h-4 w-4" /> },
      { title: 'Impressions générées', value: formatInt(current.impressions), diff: percentageDiff(current.impressions, previous.impressions), icon: <Eye className="h-4 w-4" /> },
      { title: "Analyse de l'affluence", value: formatInt(current.affluence), diff: percentageDiff(current.affluence, previous.affluence), icon: <Users className="h-4 w-4" /> },
      { title: 'Écrans actifs', value: formatInt(current.activeScreens), diff: percentageDiff(current.activeScreens, previous.activeScreens), icon: <Monitor className="h-4 w-4" /> },
      { title: 'Dépenses ce mois', value: formatCurrency(current.spend), diff: percentageDiff(current.spend, previous.spend), icon: <Wallet className="h-4 w-4" /> }
    ];
  }, [dataset]);
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-[#5C5C5C]">Analysez la performance de vos campagnes en un coup d&apos;oeil</p>
                </div>
              ...
              <div className="rounded-2xl border border-gray-200 bg-white p-5">
                ...
                <div className="rounded-2xl border border-gray-100 p-4 mb-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                    <div>
                      ...
                      <select value={filters.campaignId} onChange={(e) => onFilterChange('campaignId', e.target.value)} className="h-11 w-full rounded-lg border border-gray-200 px-3 text-sm">
                    ...
                    <div>
                      ...
                      <input type="date" value={filters.startDate} onChange={(e) => onFilterChange('startDate', e.target.value)} disabled={!showCustomDate} className="h-11 w-full rounded-lg border border-gray-200 px-3 text-sm disabled:bg-gray-50" />
                    </div>
                    <div>
                      <label className="mb-1 block text-sm font-semibold text-gray-700">Date de fin</label>
                      <input type="date" value={filters.endDate} onChange={(e) => onFilterChange('endDate', e.target.value)} disabled={!showCustomDate} className="h-11 w-full rounded-lg border border-gray-200 px-3 text-sm disabled:bg-gray-50" />
                    </div>
                    <div className="flex justify-end">
                      <button type="button" onClick={() => loadData(filters)} className="inline-flex items-center rounded-lg bg-[#76E6AB] px-4 py-2 text-sm font-medium text-[#165f2c] hover:bg-[#64d99b]">
                ...
                <div className="rounded-2xl border border-gray-100 p-4 mb-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
                    <div>
                      <label className="mb-1 block text-sm font-semibold text-gray-700">Établissement</label>
                      <select value={filters.locationId} onChange={(e) => onFilterChange('locationId', e.target.value)} className="h-11 w-full rounded-lg border border-gray-200 px-3 text-sm">
                        <option value="">Toutes</option>
                    ...
                    <div>
                      ...
                      <select value={filters.zoneId} onChange={(e) => onFilterChange('zoneId', e.target.value)} className="h-11 w-full rounded-lg border border-gray-200 px-3 text-sm">
                        <option value="">Toutes</option>
              ...
              {!loading && !error ? (
                <>
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5">
                    {kpiValues.map((kpi) => <KpiCard key={kpi.title} title={kpi.title} value={kpi.value} diff={kpi.diff} icon={kpi.icon} />)}
                  </div>

                  <div className="rounded-2xl border border-gray-200 bg-white p-4">
                    <h3 className="text-lg font-medium text-gray-900 mb-4">Filtres</h3>
                    <div className="flex flex-wrap gap-2 mb-3">
                      <button type="button" className="rounded-lg px-4 py-1.5 text-sm font-medium bg-[#dfe8ff] text-[#2c4c8a]">Evolution temporel</button>
                      <button type="button" onClick={() => setTemporalGranularity('hour')} className={`rounded-lg px-4 py-1.5 text-sm font-medium border ${temporalGranularity === 'hour' ? 'bg-white border-gray-300 text-gray-800' : 'bg-white border-gray-200 text-gray-500'}`}>Par heure</button>
                      <button type="button" onClick={() => setTemporalGranularity('day')} className={`rounded-lg px-4 py-1.5 text-sm font-medium border ${temporalGranularity === 'day' ? 'bg-white border-gray-300 text-gray-800' : 'bg-white border-gray-200 text-gray-500'}`}>Par jour</button>
                      <button type="button" onClick={() => setTemporalGranularity('week')} className={`rounded-lg px-4 py-1.5 text-sm font-medium border ${temporalGranularity === 'week' ? 'bg-white border-gray-300 text-gray-800' : 'bg-white border-gray-200 text-gray-500'}`}>Par semaine</button>
                      <button type="button" onClick={() => setTemporalGranularity('month')} className={`rounded-lg px-4 py-1.5 text-sm font-medium border ${temporalGranularity === 'month' ? 'bg-white border-gray-300 text-gray-800' : 'bg-white border-gray-200 text-gray-500'}`}>Par mois</button>
                    </div>
                    <div className="flex flex-wrap gap-2 mb-3">
                      <button type="button" className="rounded-lg px-4 py-1.5 text-sm font-medium bg-[#dfe8ff] text-[#2c4c8a]">Segmentation par âge</button>
                      <button type="button" onClick={() => setAgeSegment('u25')} className={`rounded-lg px-4 py-1.5 text-sm font-medium border ${ageSegment === 'u25' ? 'bg-white border-gray-300 text-gray-800' : 'bg-white border-gray-200 text-gray-500'}`}>- 25 ans</button>
                      <button type="button" onClick={() => setAgeSegment('25_40')} className={`rounded-lg px-4 py-1.5 text-sm font-medium border ${ageSegment === '25_40' ? 'bg-white border-gray-300 text-gray-800' : 'bg-white border-gray-200 text-gray-500'}`}>25&lt;40 ans</button>
                      <button type="button" onClick={() => setAgeSegment('40_60')} className={`rounded-lg px-4 py-1.5 text-sm font-medium border ${ageSegment === '40_60' ? 'bg-white border-gray-300 text-gray-800' : 'bg-white border-gray-200 text-gray-500'}`}>40&lt;60 ans</button>
                      <button type="button" onClick={() => setAgeSegment('60p')} className={`rounded-lg px-4 py-1.5 text-sm font-medium border ${ageSegment === '60p' ? 'bg-white border-gray-300 text-gray-800' : 'bg-white border-gray-200 text-gray-500'}`}>60 ans et +</button>
                    </div>
                    <div className="flex flex-wrap gap-2 mb-4">
                      <button type="button" className="rounded-lg px-4 py-1.5 text-sm font-medium bg-[#dfe8ff] text-[#2c4c8a]">Segmentation par sexe</button>
                      <button type="button" onClick={() => setSexSegment('male')} className={`rounded-lg px-4 py-1.5 text-sm font-medium border ${sexSegment === 'male' ? 'bg-white border-gray-300 text-gray-800' : 'bg-white border-gray-200 text-gray-500'}`}>Hommes</button>
                      <button type="button" onClick={() => setSexSegment('female')} className={`rounded-lg px-4 py-1.5 text-sm font-medium border ${sexSegment === 'female' ? 'bg-white border-gray-300 text-gray-800' : 'bg-white border-gray-200 text-gray-500'}`}>Femmes</button>
                      <button type="button" onClick={() => setSexSegment('all')} className={`rounded-lg px-4 py-1.5 text-sm font-medium border ${sexSegment === 'all' ? 'bg-white border-gray-300 text-gray-800' : 'bg-white border-gray-200 text-gray-500'}`}>Tous</button>
                      <button type="button" onClick={() => setAgeSegment('all')} className={`rounded-lg px-4 py-1.5 text-sm font-medium border ${ageSegment === 'all' ? 'bg-white border-gray-300 text-gray-800' : 'bg-white border-gray-200 text-gray-500'}`}>Tous âges</button>
                    </div>
                  ...
                  <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                    <div className="rounded-2xl border border-gray-200 bg-white p-4">
                      <h3 className="text-lg font-medium text-gray-900">Évolution de l&apos;audience</h3>
                      ...
                      <div className="h-[270px]">
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={trendData}>
                            ...
                            <Tooltip />
                            <Area type="monotone" dataKey="current" stroke="#8D95F8" strokeWidth={2} fill="url(#ownerCurrentArea)" name="2025" />
                            <Area type="monotone" dataKey="previous" stroke="#94A3B8" strokeWidth={2} fill="url(#ownerPreviousArea)" name="2026" />
                          </AreaChart>
                        </ResponsiveContainer>
                      </div>
                    </div>
                  ...
                  <button type="button" className="rounded-lg bg-[#76E6AB] px-4 py-2 text-sm font-medium text-[#165f2c] hover:bg-[#64d99b]">
                    ...
                    <div className="rounded-2xl border border-gray-200 bg-white p-4">
                      ...
                      <div className="space-y-3">
                        {(dataset?.topCampaigns || []).map((campaign, idx) => (
                          <div key={campaign.id} className="rounded-xl border border-gray-100 px-3 py-3 flex items-center justify-between">
                            <div className="flex items-center gap-3 min-w-0">
                              <div className="h-7 w-7 rounded-full bg-gray-100 flex items-center justify-center text-sm font-semibold">{idx + 1}</div>
                            ...
                            <p className="text-lg font-medium text-[#1FC16B]">{campaign.roi.toFixed(1)} TND</p>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div className="rounded-2xl border border-gray-200 bg-white p-4">
                      <h4 className="text-lg font-medium text-gray-900">Audience globale par sexe</h4>
                      ...
                      <div className="space-y-3">
                        {(dataset?.zonePerformance || []).map((zone) => (
                          <div key={zone.zoneId} className="rounded-xl border border-gray-100 p-3">
                            <div className="mb-2 flex items-center justify-between">
                              <div className="flex items-center gap-2 text-sm font-semibold"><MapPin className="h-4 w-4" />{zone.zoneName}</div>
                              <div className="text-sm text-gray-500">{zone.sharePercent.toFixed(1)}%</div>
                            </div>
                            <div className="text-sm font-semibold text-gray-900 mb-2">{formatInt(zone.impressions)} impressions</div>
                            <div className="h-2 rounded-full bg-gray-100">
                              <div className="h-2 rounded-full bg-[#9EEBB5]" style={{ width: `${Math.max(0, Math.min(100, zone.sharePercent))}%` }} />
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="rounded-2xl border border-[#96E3B0] bg-[#F5FAF8] p-4">
                    ...
                    <p className="mb-4 text-sm text-gray-500">Indicateurs de performance complémentaires</p>
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
                      <div className="rounded-xl border border-[#76E6AB] bg-white p-4">
                        ...
                        <p className="mt-2 text-2xl font-medium">{formatInt(dataset?.detailedMetrics.averageDurationDays || 0)} jours</p>
                      ...
                      <div className="rounded-xl border border-[#76E6AB] bg-white p-4">
                        ...
                        <p className="mt-2 text-2xl font-medium">{formatInt(dataset?.kpis.impressions || 0)}</p>
                        <p className="mt-2 text-sm text-gray-500">Personnes</p>
                      </div>
                      <div className="rounded-xl border border-[#76E6AB] bg-white p-4">
                        <p className="text-xs font-semibold text-gray-900">Revenu total</p>
                        <p className="mt-2 text-2xl font-medium">{formatCurrency(dataset?.detailedMetrics.totalBudget || 0)}</p>
                        <p className="mt-2 text-sm text-gray-500">En 2026</p>
                      </div>
                      <div className="rounded-xl border border-[#76E6AB] bg-white p-4">
                        <p className="text-xs font-semibold text-gray-900">Taux d&apos;occupation</p>
                        <p className="mt-2 text-2xl font-medium">{safeNumber(dataset?.detailedMetrics.completionRate || 0).toFixed(0)}%</p>
                        <p className="mt-2 text-sm text-gray-500">Ecrans disponibles</p>
                      </div>
                    </div>
                    <button type="button" className="mt-4 rounded-lg bg-[#76E6AB] px-4 py-2 text-sm font-medium text-[#165f2c] hover:bg-[#64d99b]">
