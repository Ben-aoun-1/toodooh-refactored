...
export default function SignUpForm({ currentStep, onStepChange, onProfileTypeChange }: Props) {
  ...
  // F6 — optional screenhost geo + WiFi per fleet establishment (carried into the FleetRow on add).
  // Dual-source ruling (2026-06-11): signup MAY capture coordinates; the TV's first-login GPS
  // fills only screenhosts whose lat/lng are still NULL, so signup-provided values win.
  ...
  // F6 — optional screenhost geo + WiFi for the individual owner's single location (attached at
  // submit). Dual-source ruling (2026-06-11): see the fleet-draft note above.
  ...
  // F6 — "Utiliser ma position": prefill the lat/lng inputs from the browser. Manual entry stays the
  ...
  const addFleetEstablishment = () => {
    ...
    const row: FleetRow = {
      ...
      // F6 — optional geo + WiFi ("add later"); omitted when blank (dual-source ruling 2026-06-11).
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const signupDataWithFile: SignUpData = {
        ...
        // screenhosts whose lat/lng are still NULL, so values captured here win.
        latitude:
  ...
  /* ═══════ Screenhost geo + WiFi capture (F6) — shared by the individual-owner address step and
     each fleet-establishment draft. Manual lat/lng is the baseline (works on HTTP); "Utiliser ma
     position" only prefills. Everything here is optional — nothing blocks submission. Dual-source
     ruling (2026-06-11): the TV's first-login GPS fills only NULL coordinates, so these win. ═══════ */
