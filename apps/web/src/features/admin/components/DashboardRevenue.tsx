...
import { formatHt, revenueMonthLabel } from '@/features/admin/lib/admin-dashboard';
...
//   « Revenu Toodooh » (was « Revenu Total », a sum of confirmed RECHARGES) = Toodooh's share of
//   the settlements; « Revenu mensuel » = both figures over the current Tunis month; « Revenu
//   total » (was « Budget Campagnes », the requested budgets of every status) = settled spend.
// The formulas live in the api (routes/admin-platform-stats.ts); this only renders them.
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-sm p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold mt-2">{formatHt(revenue?.toodooh_tnd ?? 0)}</p>
            </div>
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-600">
                Revenu mensuel · {revenueMonthLabel(revenue?.monthly.month)}
              </p>
              <dl className="mt-2 space-y-1">
                <div className="flex items-baseline justify-between gap-2">
                  <dt className="text-sm text-gray-600">Total</dt>
                  <dd className="text-lg font-bold text-gray-900">
                    {formatHt(revenue?.monthly.total_tnd ?? 0)}
                  </dd>
                </div>
                <div className="flex items-baseline justify-between gap-2">
                  <dt className="text-sm text-gray-600">Toodooh</dt>
                  <dd className="text-lg font-bold text-gray-900">
                    {formatHt(revenue?.monthly.toodooh_tnd ?? 0)}
                  </dd>
                </div>
              </dl>
            </div>
