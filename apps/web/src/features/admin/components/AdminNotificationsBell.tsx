...
// ADM-BELL1 — the admin bell: the advertiser bell's structure on the admin feed, no auto-open (an
// admin lands on a page to work, not to be popped at) and no settings footer (no admin equivalent).
export default function AdminNotificationsBell({ userId, emphasized = false }: Props) {
  ...
  const { items, readIds, loading, refetch, markRead, markAllRead } = useAdminNotifications(userId);
      ...
      {open ? (
        <div className="absolute right-0 mt-2 w-[560px] max-w-[calc(100vw-24px)] rounded-2xl border border-gray-200 bg-white shadow-xl z-[90] overflow-hidden">
          ...
          <div className="px-5 py-3 border-t border-gray-100 flex items-center justify-between">
            <span className="text-sm text-[#5C5C5C]">File d’attente administrateur</span>
            <button
              type="button"
