import { Video, VideoValidationStats, CampaignUsingVideo } from '@/features/admin/types/video';
...
export const adminVideoService = {
  ...
  async approveVideo(videoId: string, adminId: string, notes?: string): Promise<boolean> {
    ...
    const { error } = await supabase.from('videos').update(updateData).eq('id', videoId).select();
