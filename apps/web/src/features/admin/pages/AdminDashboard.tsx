...
import DashboardRevenue from '@/features/admin/components/DashboardRevenue';
...
import {
  ...
  campaignQueueHref,
  formatHt,
} from '@/features/admin/lib/admin-dashboard';
import { CAMPAIGN_QUEUE_ALL } from '@/features/admin/lib/campaign-queue';
...
// DASH-1 (operator rulings R1–R6, 2026-09-21): the formulas live in the api; labels, links and money
// formatting live in lib/admin-dashboard.ts. The recharge-based figures are gone (not revenue).
export default function AdminDashboard() {
  ...
  return (
    <AdminLayout title="Tableau de Bord">
      ...
      <DashboardRevenue revenue={revenueStats} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div>
          ...
          {/* R4 — « Actifs » read is_active, which nothing writes: installed + online replace it. */}
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">Installés</p>
                  <p className="text-2xl font-bold text-blue-600">{screensStats?.installed ?? 0}</p>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">{screensStats?.online ?? 0}</p>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <button
              onClick={() => navigate(campaignQueueHref(CAMPAIGN_QUEUE_ALL))}
              className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 hover:shadow-md hover:border-brand-primary transition-all cursor-pointer text-left w-full"
            >
              <div className="flex items-center justify-between">
                <div>
            ...
            <button
              onClick={() => navigate(campaignQueueHref('active'))}
            ...
            <button
              onClick={() => navigate(campaignQueueHref('pending'))}
              className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 hover:shadow-md hover:border-yellow-400 transition-all cursor-pointer text-left w-full"
            >
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">
                    {campaignFilterLabel('pending')}
                  </p>
            ...
            {/* ADM-FIX1 — « À venir » and « Passées » are STORED statuses (CF-S1) the dashboard
                never surfaced, although `total` counted them. Both deep-link into the queue. */}
            <button
              onClick={() => navigate(campaignQueueHref('upcoming'))}
            ...
            <button
              onClick={() => navigate(campaignQueueHref('completed'))}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {formatHt(campaignsStats?.average_budget_tnd ?? 0)}
                  </p>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          {/* R6 — all six statuses, lifecycle order, the queue's own labels. */}
          <div className="space-y-3">
            {campaignActivityRows(campaignsStats).map((row) => (
              <div key={row.status} className="flex justify-between items-center">
                <span className="text-sm text-gray-600">{row.label}</span>
                <span className={`font-semibold ${row.valueClass}`}>{row.count}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600">Écrans installés</span>
              <span className="font-semibold text-gray-900">{screensStats?.installed ?? 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600">Écrans en ligne</span>
              <span className="font-semibold text-gray-900">{screensStats?.online ?? 0}</span>
            </div>
            <div className="flex justify-between items-center">
