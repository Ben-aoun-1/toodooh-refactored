                  ...
                  <div>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex flex-col gap-2">
                        {/* Phase-1f F7b — logo upload/remove disabled (D-F4-4): no backend logo
                            storage yet; deferred to a later "logo storage" slice. Path severed
                            (handlers + Supabase chain removed); the static disabled markup +
                            caption is the honest interim UX. */}
                ...
                {(documentConfirmed || documentFile) && (
                  <div className="flex items-center gap-4 p-4 bg-white rounded-xl border border-gray-200 shadow-sm">
                    ...
                    {/* Phase-1f F7b — clears a freshly-picked local file; removing an UPLOADED
                        ...
                        D-F5-3 defer wired: the dead else-branch is gone; disabled gates this
                        button when no local file is present. */}
                    <button
                      type="button"
          ...
          <div className="flex-1 min-w-0 bg-white overflow-hidden">
            ...
            {confidentialiteSub === 'delete' && (
              <div className="p-6 max-w-xl space-y-4">
                {/* Phase-1f F7b — D-F7-2 ruling: deactivation control disabled until the backend
                    /api/account/deactivate endpoint ships. Pre-K the form re-auth-confirmed via
                    supabase.auth.signInWithPassword before destroying the account; post-K the
                    Supabase auth store is no longer the password source (users live in
                    apps/api's users table), so the re-auth was a broken no-op that rejected
                    every password attempt — a class-(b) security gap. The static message below
                    is the honest interim path while preserving the user's right to close their
