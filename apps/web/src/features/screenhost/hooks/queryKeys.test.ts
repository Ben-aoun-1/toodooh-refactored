...
describe('screenhostKeys', () => {
  it('namespaces every view key under the "screenhost" prefix', () => {
    ...
    expect(screenhostKeys.campaigns('u1')[0]).toBe('screenhost');
  });

  it('keys the campaigns overview per user for cache isolation', () => {
    expect(screenhostKeys.campaigns('u1')).toEqual(['screenhost', 'campaigns', 'u1']);
    expect(screenhostKeys.campaigns('u1')).not.toEqual(screenhostKeys.campaigns('u2'));
  });
  ...
  it('keys the notification feed per user', () => {
    ...
    expect(screenhostKeys.notifications('u1')).not.toEqual(screenhostKeys.campaigns('u1'));
  });

  it('keeps every view key prefix-matchable by screenhostKeys.all', () => {
    for (const key of [screenhostKeys.campaigns('u1'), screenhostKeys.notifications('u1')]) {
