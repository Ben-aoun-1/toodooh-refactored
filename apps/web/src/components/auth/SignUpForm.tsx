...
import { Eye, EyeOff, Mail, Lock, Phone, Upload, FileText, X, CheckCircle, User, Building2, MapPin, MoreVertical, Trash2 } from 'lucide-react';
import { toast } from 'react-hot-toast';
import { authService } from '../../services/auth.service';
import type { BusinessSector, Governorate, SignUpData, SignUpResult, FleetEstablishmentInput, CompanySizeOption } from '../../types/auth';
import { AGENCY_BUSINESS_SECTOR_NAME, sectorsForAdvertiserAgencySignup } from '../../constants/advertiserBusinessSectors';
...
const profileCards: { id: ProfileType; title: string; description: string; icon: string; iconSelected: string; group: 'visibility' | 'revenue' }[] = [
  { id: 'advertiser', title: 'Annonceur', description: 'Je suis une entreprise et je fais de la publicité pour ma marque', icon: annonceurIcon, iconSelected: annonceurIconS, group: 'visibility' },
  { id: 'agency', title: 'Agence', description: 'Je suis une agence et je gère des campagnes pour mes clients', icon: agenceIcon, iconSelected: agenceIconS, group: 'visibility' },
  { id: 'individual_owner', title: 'Propriétaire individuel', description: 'Je monétise mon écran dans mon établissement', icon: individuelIcon, iconSelected: individuelIconS, group: 'revenue' },
  { id: 'fleet_owner', title: 'Propriétaire de parc', description: 'Je monétise mes écrans dans mon réseau d\'établissements', icon: parcIcon, iconSelected: parcIconS, group: 'revenue' },
];

const ownerZones = [
  'La Marsa (Banlieue Nord)', 'Jardins de Carthage', 'Aouina/Ain Zaghouan',
  'Berges du Lac', 'Ariana', 'El Menzah/El Manar', 'Centre Ville Tunis',
  'Rades/Megrine/Ben Arous/Banlieue Sud', 'Manouba'
];
...
const tunisianCities = [
  ...
  'Sfax', 'Gafsa', 'Tozeur', 'Kebili', 'Tataouine', 'Médenine', 'Gabès',
];
...
const formuleOptions = [
  { id: 'abonnement', title: 'Abonnement', description: 'Toodooh prend en charge votre abonnement IPTV, Diwan Sport, etc., sans aucun frais', icon: '📺' },
  { id: 'revenue_share', title: 'Revenue share', description: 'Générez des revenus variables en recevant une part sur les publicités diffusées', icon: '🤝' },
];
...
export default function SignUpForm({ currentStep, onStepChange, onProfileTypeChange }: Props) {
  ...
  const [formData, setFormData] = useState<Partial<SignUpData>>({
    email: '', password: '', business_name: '', tax_number: '',
    business_sector_id: '', business_type: 'local', profile_type: 'advertiser',
    contact_name: '', contact_phone: '+216', street_address: '', city: '',
    postal_code: '', governorate_id: '', zone: '', cin: '', formule: '',
    agent_toodooh: '', number_of_screens: undefined, number_of_rooms: undefined, company_size: '', terms_accepted: false,
  });

  const isOwner = selectedProfileType === 'individual_owner' || selectedProfileType === 'fleet_owner';
  const prevProfileTypeRef = useRef<ProfileType | null>(null);

  useEffect(() => {
    const loadData = async () => {
      try {
        const [sectorsData, ownerSectorsData, companySizeData, governoratesData] = await Promise.all([
  ...
  const validatePassword = (pw: string) => pw.length >= 8 && /[A-Z]/.test(pw) && /[a-z]/.test(pw) && /\d/.test(pw);
  ...
  const pwMatch = Boolean(formData.password && confirmPassword && formData.password === confirmPassword);
  ...
  const validateUniqueCredentials = async (
    ...
    try {
      const { emailExists } = await authService.checkSignupConflicts(normalizedEmail, normalizedPhone);
      const nextEmailConflict = checkEmail && emailExists ? 'Cette adresse email existe déjà' : null;
      setEmailConflict(nextEmailConflict);
      setPhoneConflict(null);

      if (nextEmailConflict) {
        if (showToast) toast.error('📧 Cette adresse email est déjà utilisée. Veuillez vous connecter ou utiliser une autre adresse.');
        return false;
      }
      return true;
    } catch {
  ...
  const canGoNext = () => {
    switch (currentStep) {
      case 0: return Boolean(selectedProfileType);
      case 1:
        return Boolean(
          lastName.trim() &&
            firstName.trim() &&
      ...
      case 3:
        ...
        return Boolean(formData.street_address?.trim() && formData.city?.trim() && formData.postal_code?.trim() && formData.governorate_id);
      case 4: return true;
  ...
  const goNext = () => {
    ...
    if (selectedProfileType === 'individual_owner' && currentStep === 2) {
      const screensNum = etablissementScreens === '6-10' ? 8 : etablissementScreens === '10+' ? 10 : parseInt(etablissementScreens, 10);
      setFormData((prev) => ({
        ...prev,
  ...
  /* ── render helpers ── */
  const inputClass = 'w-full px-4 py-3 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#00B3A6]/30 focus:border-[#00B3A6] transition-all text-sm text-gray-900 placeholder-gray-400';
  ...
  const renderProfileSelection = () => {
    ...
    const renderGroup = (label: string, cards: typeof profileCards, groupType: 'visibility' | 'revenue') => {
      const colors = groupColors[groupType];
      return (
        <div className="rounded-3xl p-3 pt-6" style={{ background: colors.bg }}>
          <p className="text-center font-medium text-lg mb-5" style={{ color: colors.title, letterSpacing: '-0.006em' }}>{label}</p>
                ...
                >
                  ...
                  <div className="absolute top-4 right-4">
                    {isSelected ? (
                      ...
                      <div className="w-5 h-5 rounded-full flex items-center justify-center" style={{ background: '#EBEBEB' }}>
                        <div className="w-[13px] h-[13px] rounded-full bg-white" style={{ boxShadow: '0px 2px 2px rgba(27, 28, 29, 0.12)' }} />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-1">
                    <h4 className="font-medium text-sm" style={{ color: '#171717', letterSpacing: '-0.006em' }}>{card.title}</h4>
                    <p className="text-xs leading-4" style={{ color: '#5C5C5C', letterSpacing: '-0.006em' }}>{card.description}</p>
                  </div>
                </button>
              );
            })}
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-10">
        ...
        <p className="text-sm text-gray-500 mb-8">Sélectionnez le type de profil qui correspond le mieux à votre activité</p>
    ...
    <div className="max-w-3xl mx-auto w-full space-y-5">
      <div className="text-center mb-6">
        ...
        <h2 className="text-xl font-bold text-gray-900">
          {selectedProfileType === 'individual_owner' ? "Information sur l'agent" : 'Informations sur le responsable'}
        </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div>
          ...
          <input type="text" required value={lastName} onChange={(e) => setLastName(e.target.value)} className={inputClass} placeholder="Nom" />
        </div>
        {/* Prénom */}
        <div>
          <label className={labelClass}>Prénom <span className="text-red-500">*</span></label>
          <input type="text" required value={firstName} onChange={(e) => setFirstName(e.target.value)} className={inputClass} placeholder="Prénom" />
        </div>
        {/* Fonction */}
        <div>
          <label className={labelClass}>Fonction <span className="text-red-500">*</span></label>
          <input
            type="text"
            required
            value={fonction}
        ...
        {/* Email */}
        <div>
          <label className={labelClass}>Email professionnel <span className="text-red-500">*</span></label>
          <input
            type="email"
            required
            ...
            onBlur={() => { if (formData.email?.trim()) void validateUniqueCredentials(false, 'email'); }}
            className={inputClass}
        ...
        <div>
          ...
          <input type="tel" required value={formData.contact_phone} onChange={(e) => {
            setPhoneConflict(null);
          ...
          }} onBlur={() => { if (formData.contact_phone?.trim()) void validateUniqueCredentials(false, 'phone'); }} className={inputClass} placeholder="+21612345678" />
        ...
        <div>
          <label className={labelClass}>
            CODE DE VOTRE {isOwner ? 'SCREENHOST' : 'SCREENCAST'} AGENT <span className="text-red-500">*</span>
          </label>
          <input
            type="text"
            required
        ...
        <div>
          ...
          <input type={showPassword ? 'text' : 'password'} required value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} className={inputClass} placeholder="••••••••••" />
        </div>
        {/* Confirmer mot de passe */}
        <div>
          <label className={labelClass}>Confirmer Mot de passe <span className="text-red-500">*</span></label>
          <input type={showConfirmPassword ? 'text' : 'password'} required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className={inputClass} placeholder="••••••••••" />
        </div>
      </div>
      ...
      <div className="space-y-1 pt-1">
        ...
        <div className="flex items-center gap-1.5">
          <div className={`w-4 h-4 rounded-full flex items-center justify-center ${pwHasUpper ? 'bg-[#00B3A6]' : 'bg-gray-200'}`}>
            {pwHasUpper && <svg width="10" height="10" viewBox="0 0 12 12" fill="none"><path d="M2.5 6.5L4.5 8.5L9.5 3.5" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>}
          </div>
          <span className={`text-xs ${pwHasUpper ? 'text-gray-700' : 'text-gray-400'}`}>Au moins une majuscule</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className={`w-4 h-4 rounded-full flex items-center justify-center ${pwHasDigit ? 'bg-[#00B3A6]' : 'bg-gray-200'}`}>
            {pwHasDigit && <svg width="10" height="10" viewBox="0 0 12 12" fill="none"><path d="M2.5 6.5L4.5 8.5L9.5 3.5" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>}
          </div>
          <span className={`text-xs ${pwHasDigit ? 'text-gray-700' : 'text-gray-400'}`}>Au moins un chiffre</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className={`w-4 h-4 rounded-full flex items-center justify-center ${pwHasMinLen ? 'bg-[#00B3A6]' : 'bg-gray-200'}`}>
            {pwHasMinLen && <svg width="10" height="10" viewBox="0 0 12 12" fill="none"><path d="M2.5 6.5L4.5 8.5L9.5 3.5" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>}
          </div>
          <span className={`text-xs ${pwHasMinLen ? 'text-gray-700' : 'text-gray-400'}`}>Minimum 8 caractères</span>
        </div>
      </div>
    </div>
  );
    ...
    <div className="max-w-3xl mx-auto w-full space-y-5">
        ...
        <>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 ml-auto flex-shrink-0">
              <div className="rounded-full border border-gray-200 bg-gray-50 flex items-center justify-center overflow-hidden" style={{ width: 56, height: 56 }}>
                {companyLogoPreview ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className={labelClass}>Nom de l&apos;établissement <span className="text-red-500">*</span></label>
              <input
                type="text"
                required
            ...
            <div>
              <label className={labelClass}>Matricule fiscal <span className="text-red-500">*</span></label>
              <input type="text" required value={formData.tax_number} onChange={(e) => setFormData({ ...formData, tax_number: e.target.value })} className={inputClass} placeholder="Matricule fiscal" />
            </div>
            <div>
              <label className={labelClass}>Catégorie <span className="text-red-500">*</span></label>
              <select required value={formData.business_sector_id} onChange={(e) => setFormData({ ...formData, business_sector_id: e.target.value })} className={inputClass}>
            ...
            <div>
              <label className={labelClass}>Nombre d&apos;écrans <span className="text-red-500">*</span></label>
              <select required value={etablissementScreens} onChange={(e) => setEtablissementScreens(e.target.value)} className={inputClass}>
                {screenOptions.map((o) => <option key={`io-${o.value || 'empty'}`} value={o.value}>{o.label}</option>)}
              </select>
            </div>
            <div>
              <label className={labelClass}>Nombre de salles <span className="text-red-500">*</span></label>
              <input type="text" required value={etablissementRooms} onChange={(e) => setEtablissementRooms(e.target.value)} className={inputClass} placeholder="2" />
            </div>
          </div>
        </>
      ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div>
          ...
          <input type="text" required value={formData.business_name} onChange={(e) => setFormData({ ...formData, business_name: e.target.value })} className={inputClass} placeholder="Nom de l'entreprise" />
        </div>
        <div>
          <label className={labelClass}>Matricule fiscal <span className="text-red-500">*</span></label>
          <input type="text" required value={formData.tax_number} onChange={(e) => setFormData({ ...formData, tax_number: e.target.value })} className={inputClass} placeholder="Matricule fiscal" />
        ...
        <div>
          <label className={labelClass}>
            {isOwner ? "Nombre d'établissements de votre parc" : "Taille de l'entreprise"} <span className="text-red-500">*</span>
          </label>
          <select required value={formData.company_size || ''} onChange={(e) => setFormData({ ...formData, company_size: e.target.value })} className={inputClass}>
            <option value="">{isOwner ? '12' : "Sélectionnez la taille"}</option>
            {isOwner
              ? parcCountOptions.map((n) => <option key={n} value={n}>{n}</option>)
              : companySizeOptions.map((o) => <option key={o.id} value={o.value}>{o.value}</option>)}
          </select>
        </div>
      </div>

      <div>
        <label className={labelClass}>Adresse du siège <span className="text-red-500">*</span></label>
        <input type="text" required value={formData.street_address} onChange={(e) => setFormData({ ...formData, street_address: e.target.value })} className={inputClass} placeholder="Adresse" />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div>
          <label className={labelClass}>Ville <span className="text-red-500">*</span></label>
          <input type="text" required value={formData.city} onChange={(e) => setFormData({ ...formData, city: e.target.value })} className={inputClass} placeholder="Ville" />
        </div>
        <div>
          <label className={labelClass}>Zone <span className="text-red-500">*</span></label>
          <input type="text" required value={formData.zone} onChange={(e) => setFormData({ ...formData, zone: e.target.value })} className={inputClass} placeholder="1000" />
        </div>
      </div>
      ...
      <div>
        <label className={labelClass}>Gouvernorat <span className="text-red-500">*</span></label>
        <select required value={formData.governorate_id} onChange={(e) => setFormData({ ...formData, governorate_id: e.target.value })} className={inputClass}>
  ...
  const renderEtablissementIndividual = () => (
    <div className="max-w-3xl mx-auto w-full space-y-5">
      ...
      <div className="space-y-5">
        <div>
          <label className={labelClass}>Nom de l&apos;établissement <span className="text-red-500">*</span></label>
          <input type="text" required value={etablissementName} onChange={(e) => setEtablissementName(e.target.value)} className={inputClass} placeholder="Nom de l'établissement" />
        </div>
        <div>
          <label className={labelClass}>Nombre d&apos;écrans <span className="text-red-500">*</span></label>
          <select required value={etablissementScreens} onChange={(e) => setEtablissementScreens(e.target.value)} className={inputClass}>
            {screenOptions.map((o) => <option key={o.value || 'empty'} value={o.value}>{o.label}</option>)}
          </select>
        </div>
        <div>
          <label className={labelClass}>Nombre de salles <span className="text-red-500">*</span></label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            <label className={labelClass}>Zone <span className="text-red-500">*</span></label>
            <input type="text" required value={formData.zone} onChange={(e) => setFormData({ ...formData, zone: e.target.value })} className={inputClass} placeholder="Zone" />
          </div>
        </div>
        <div>
          <label className={labelClass}>Gouvernorat <span className="text-red-500">*</span></label>
  ...
  const renderEtablissementFleet = () => (
    <div className="max-w-3xl mx-auto w-full space-y-6">
              ...
              <tbody className="divide-y divide-gray-100">
                {fleetEstablishments.map((row) => (
                  <tr key={row.id} className="hover:bg-gray-50/80">
                    ...
                    <td className="px-4 py-3 text-center tabular-nums text-gray-800">{row.screen_count}</td>
                    <td className="px-4 py-3 text-center tabular-nums text-gray-800">{row.room_count}</td>
      ...
      <div className="space-y-5">
        ...
        <div>
          <label className={labelClass}>Nombre d&apos;écrans <span className="text-red-500">*</span></label>
          <select value={fleetDraftScreens} onChange={(e) => setFleetDraftScreens(e.target.value)} className={inputClass}>
            {screenOptions.map((o) => <option key={`f-${o.value || 'e'}`} value={o.value}>{o.label}</option>)}
          </select>
        </div>
        <div>
          <label className={labelClass}>Nombre de salles <span className="text-red-500">*</span></label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            <label className={labelClass}>Ville <span className="text-red-500">*</span></label>
            <select value={fleetDraftCity} onChange={(e) => setFleetDraftCity(e.target.value)} className={inputClass}>
        ...
        <div>
          <label className={labelClass}>Gouvernorat <span className="text-red-500">*</span></label>
          <select value={fleetDraftGovernorate} onChange={(e) => setFleetDraftGovernorate(e.target.value)} className={inputClass}>
            <option value="">Gouvernorat</option>
            {governorates.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}
          </select>
        </div>
  ...
  const renderEtablissement = () =>
    selectedProfileType === 'fleet_owner' ? renderEtablissementFleet() : renderEtablissementIndividual();
  ...
  const renderCoordonneesBancaires = () => (
    <div className="max-w-3xl mx-auto w-full space-y-6">
      <div className="text-center mb-2">
        ...
        <p className="text-sm text-gray-500">Téléchargez vos coordonnées bancaires pour recevoir les revenus de vos campagnes</p>
      </div>

      <div className="rounded-2xl p-5" style={{ background: '#F5F5F5' }}>
        <label className="flex items-start cursor-pointer gap-3">
          <input type="checkbox" checked={addBankLater} onChange={(e) => { setAddBankLater(e.target.checked); if (e.target.checked) setBankDocFile(null); }} className="h-5 w-5 text-[#00B3A6] focus:ring-[#00B3A6] border-gray-300 rounded mt-0.5 flex-shrink-0" />
          <div>
            <p className="text-sm font-semibold text-gray-900">J&apos;ajouterai mes coordonnées bancaires plus tard</p>
            <p className="text-xs text-gray-500 mt-1">Vous pourrez uploader vos documents depuis votre profil après inscription.</p>
          </div>
        </label>
      </div>

      {!addBankLater && (
        <div className="border-2 border-dashed border-gray-300 rounded-2xl p-8 text-center" style={{ background: '#FAFAFA' }}>
          ...
          <p className="text-xs text-gray-500 mb-5">Formats acceptés : PDF, JPG, JPEG, PNG (Max 5 MB)</p>
          ...
          {bankDocFile && (
            <p className="mt-3 text-sm text-gray-600">{bankDocFile.name}</p>
          )}
        </div>
      )}
      ...
      <div className="space-y-4 pt-2">
        <label className="flex items-start cursor-pointer gap-3">
          <input type="checkbox" checked={formData.terms_accepted} onChange={(e) => setFormData({ ...formData, terms_accepted: e.target.checked })} className="h-5 w-5 text-[#00B3A6] focus:ring-[#00B3A6] border-gray-300 rounded mt-0.5 flex-shrink-0" />
          <span className="text-sm text-gray-700">J&apos;accepte <a href="/terms" className="text-gray-900 font-medium underline underline-offset-2">les conditions générales d&apos;utilisation</a></span>
        </label>
        <label className="flex items-start cursor-pointer gap-3">
          <input type="checkbox" checked={ownerCertificationAccepted} onChange={(e) => setOwnerCertificationAccepted(e.target.checked)} className="h-5 w-5 text-[#00B3A6] focus:ring-[#00B3A6] border-gray-300 rounded mt-0.5 flex-shrink-0" />
          <span className="text-sm text-gray-500">En validant mon inscription, je certifie être autorisé(e) à inscrire cet établissement sur Toodooh,</span>
        </label>
      </div>
    </div>
  );
    ...
    <div className="max-w-3xl mx-auto w-full space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div className="md:col-span-2">
          <label className={labelClass}>Adresse <span className="text-red-500">*</span></label>
          <input type="text" required value={formData.street_address} onChange={(e) => setFormData({ ...formData, street_address: e.target.value })} className={inputClass} placeholder="123 Rue de la Paix" />
        </div>
        <div>
          <label className={labelClass}>Ville <span className="text-red-500">*</span></label>
          <input type="text" required value={formData.city} onChange={(e) => setFormData({ ...formData, city: e.target.value })} className={inputClass} placeholder="Tunis" />
        </div>
        <div>
          <label className={labelClass}>Code postal <span className="text-red-500">*</span></label>
          <input type="text" required pattern="\d{4}" value={formData.postal_code} onChange={(e) => setFormData({ ...formData, postal_code: e.target.value })} className={inputClass} placeholder="1000" />
        </div>
        {(selectedProfileType === 'fleet_owner' || selectedProfileType === 'individual_owner') && (
          <div className="md:col-span-2">
            <label className={labelClass}>Secteur/Zone <span className="text-red-500">*</span></label>
            <select required value={formData.zone} onChange={(e) => setFormData({ ...formData, zone: e.target.value })} className={inputClass}>
              <option value="">-- Choisissez --</option>
              {ownerZones.map((z, i) => <option key={i} value={z}>{z}</option>)}
            </select>
          </div>
        )}
        <div className="md:col-span-2">
  ...
  const renderStep4 = () => (
    <div className="max-w-3xl mx-auto w-full space-y-6">
      ...
      <div className="rounded-2xl p-5" style={{ background: '#F5F5F5' }}>
        <label className="flex items-start cursor-pointer gap-3">
          <input type="checkbox" checked={addDocumentLater} onChange={(e) => { setAddDocumentLater(e.target.checked); if (e.target.checked) setDocumentFile(null); }} className="h-5 w-5 text-[#00B3A6] focus:ring-[#00B3A6] border-gray-300 rounded mt-0.5 flex-shrink-0" />
          <div>
            <p className="text-sm font-semibold text-gray-900">J&apos;ajouterai mes documents plus tard</p>
            <p className="text-xs text-gray-500 mt-1">Vous pourrez uploader vos documents depuis votre profil après inscription.</p>
            <p className="text-xs text-gray-500">Vous ne pourrez lancer votre première campagne une fois tous les documents téléchargés et validés.</p>
          </div>
        </label>
      </div>
        ...
        <div className="space-y-4">
          <div className="border-2 border-dashed border-gray-300 rounded-2xl p-8 text-center" style={{ background: '#FAFAFA' }}>
            ...
            <label className="inline-block cursor-pointer px-6 py-2.5 border border-gray-300 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
              ...
              <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" onChange={(e) => {
                if (e.target.files?.[0]) {
                  if (e.target.files[0].size > 5 * 1024 * 1024) { toast.error('Fichier trop volumineux (max 5 MB)'); return; }
            ...
            <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-green-700">{(documentFile.size / 1024 / 1024).toFixed(2)} MB</p>
                </div>
              </div>
              <button type="button" onClick={() => setDocumentFile(null)} className="flex items-center px-3 py-1.5 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 text-sm">
      ...
      <div className="space-y-4 pt-2">
        ...
        <label className="flex items-start cursor-pointer gap-3">
          <input type="checkbox" className="h-5 w-5 text-[#00B3A6] focus:ring-[#00B3A6] border-gray-300 rounded mt-0.5 flex-shrink-0" />
          <span className="text-sm text-gray-500">En validant mon inscription, je certifie être autorisé(e) à inscrire cette entreprise sur Toodooh, et à engager des dépenses publicitaires en son nom</span>
        </label>
      </div>
    </div>
  );
  ...
  const stepContent = () => {
    switch (currentStep) {
      case 0: return renderProfileSelection();
      case 1: return renderStep1();
      case 2: return renderStep2();
      case 3:
        if (selectedProfileType === 'fleet_owner') return renderEtablissement();
        return renderStep3();
      case 4: return isOwner ? renderCoordonneesBancaires() : renderStep4();
      default: return null;
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col px-4 py-8">
      ...
      <div className="max-w-3xl mx-auto w-full mt-8 flex items-center gap-4">
        <button
          type="button"
          onClick={() => { if (currentStep === 0) navigate('/login'); else goPrev(); }}
        ...
        {currentStep < 4 ? (
          ...
          <button
            ...
            disabled={loading || !formData.terms_accepted || (isOwner && !ownerCertificationAccepted)}
            className="flex-1 py-3.5 rounded-xl font-semibold text-sm text-white transition-all disabled:opacity-40 disabled:cursor-not-allowed"
            style={{ background: '#76E6AB' }}
          >
            {loading ? (
