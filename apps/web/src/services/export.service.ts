...
class ExportService {
  ...
  async exportToPDF(
    ...
    try {
      ...
      if (type === 'revenue') {
        ...
        autoTable(doc, {
          head: [['Écran', 'Emplacement', 'Revenu Total (TND)', 'Revenu Mensuel (TND)', 'Moyenne (TND)']],
      ...
      doc.setFontSize(10);
      doc.setTextColor(100, 100, 100);
      const totalRevenue = data.reduce((sum: number, item: any) => sum + (item.amount || item.total_revenue), 0);
  ...
  async exportToExcel(
    ...
    try {
      ...
      if (type === 'revenue') {
        ...
        const screenData = data as ScreenRevenue[];
        headers = ['Écran', 'Emplacement', 'Revenu Total (TND)', 'Revenu Mensuel (TND)', 'Moyenne (TND)'];
      ...
      const summaryData = [
        ...
        ['Revenus totaux', data.reduce((sum: number, item: any) => sum + (item.amount || item.total_revenue || item.revenue), 0)],
        [''],
        ['Statistiques'],
        ['Revenu moyen', data.reduce((sum: number, item: any) => sum + (item.amount || item.total_revenue || item.revenue), 0) / data.length],
        ['Revenu maximum', Math.max(...data.map((item: any) => item.amount || item.total_revenue || item.revenue))],
        ['Revenu minimum', Math.min(...data.map((item: any) => item.amount || item.total_revenue || item.revenue))]
      ];
  ...
  async exportAllData(
    ...
    try {
      ...
      const screenHeaders = ['Écran', 'Emplacement', 'Revenu Total (TND)', 'Revenu Mensuel (TND)', 'Moyenne (TND)'];
      ...
      const summaryData = [
        ...
        ['Revenu moyen par écran', screenData.reduce((sum, item) => sum + item.total_revenue, 0) / screenData.length],
        [''],
        ['Performance par Écran'],
        ['Écran le plus performant', screenData.sort((a, b) => b.total_revenue - a.total_revenue)[0]?.screen_name || 'N/A'],
