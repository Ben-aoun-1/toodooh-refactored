...
describe('owner performance reads (owner-scoped, real Postgres)', () => {
  ...
  describe('GET /api/screenhosts/:id/audience', () => {
    ...
    it('PAX day wins, backup grid otherwise — with per-day provenance on the wire', async () => {
      ...
      // FLOW-4 — the grid HOUR fills both halves, whose mean is that hour again, so the silent
      // Tuesday is worth 80 (FLOW-1 read 160). The measured Monday arrives at DAY granularity from
      // monthly_stats and is not refolded: what this test pins — the measure winning over the grid
      // — is unchanged.
      ...
      // audience was guessed on a période that measured 500 of its 580. FLOW-4 halves the cell
      // side of this ratio while the day-granularity 500 stands, so the share drops 24 % → 14 %.
      expect(body.estimated_pct).toBe(14); // 80 / 580 = 13.8 %
    });
    ...
    it('a venue with ZERO readings serves the full backup estimate — never silent-zeros', async () => {
      ...
      // FLOW-4: 80 in each half of 10h → an hour of 80 → 80 for the day (FLOW-1 read 160).
    ...
    it('the backup grid never answers for days BEFORE the venue was onboarded', async () => {
      ...
      expect(body.days).toEqual([
        { date: '2026-08-31', audience: 1396, source: 'estimated', has_measured: false }, // the 13h HOUR
      ]);
      expect(body.total_audience).toBe(1396); // ONE Monday, not 4 — the floor is what this pins
    ...
    it('the all-time range the hero uses is floored at onboarding too', async () => {
      ...
      expect(body.days.every((d) => d.date >= '2026-08-26')).toBe(true);
      // …and the total is the floored window, not 2 435 days of it. FLOW-4: both halves of 10h
      // hold 80, so the hour — and the day — is 80.
