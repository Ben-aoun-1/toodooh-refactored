...
## Pre-extraction Q&A surfaces runtime-derived deps

Inventory-phase prompts that target extraction work should include a Q&A round on runtime-derived dependencies — the lift-vs-pull decisions on cross-step consumers, whether a memo's reads have parent-level subscribers outside the JSX being extracted, whether server data has multiple consumers that constrain where loading can move. The extractor cannot see these dependencies by reading the file alone; the user holds the context. Asking targeted questions ("does `selectedExistingVideo` have consumers other than Step 5's JSX?", "where is the cart-add side effect chain wired?") before drafting the props interface prevents Phase 2 from discovering mid-extraction that a memo can't move because it has parent-level subscribers. Step 7's Commits 9-11 inventory questions surfaced 3-5 such dependencies per commit that shaped Decisions B/C/E. See `docs/audit.md` §3 Methodology learnings (Cluster A) for the full Step-7 catalog.

## Setter-shim pattern for migrating per-field useState to unified state

When migrating from many per-field `useState` calls to a single `useState<StateObject>` or equivalent (e.g., adopting a state-machine hook in a wizard), wrap each migrated field's setter in a one-time `useCallback` shim that preserves the existing `setX(value|updater)` API: `const setCampaignName = useCallback((next) => setState(prev => ({ ...prev, campaignName: typeof next === 'function' ? next(prev.campaignName) : next })), [setState])`. This is a one-time architectural investment that zero-touch propagates to every subsequent extraction — existing `setX(value)` and `setX(prev => ...)` call sites continue to work without rewriting, even when later commits move them into extracted components as props. Step 7 Commit 6 introduced 16 setter shims for `WizardState`; Commits 6-12 reused them without touching a single call-site signature. See `docs/audit.md` §3 (Cluster B) for the worked example.

## Budget for cascading dead-code in every extraction commit

Step extractions reliably surface transitively dead code (handlers/state/imports whose only consumers were inside the extracted JSX). Don't scope an extraction commit as "just move the JSX out" — include setter-to-true verification of every modal/overlay/conditional surface the extraction touches, expect to delete handlers + transient useState slots + lucide imports + dead useEffect calls that were consumed only by the extracted region. Net deletion often exceeds net JSX-line move by 2-3×. Step 7 Commit 9 (Step 4 extraction) produced -929 lines from NewCampaign.tsx via Leaflet extraction + dead zone modal deletion; Commit 12 (orchestrator cleanup) deleted Events modal + hidden sidebar + formData facade for additional -271 net. See `docs/audit.md` §3 (Cluster A).

## Dead-UI detection requires TWO complementary patterns

Audits for dead UI typically scan for `{cond && <JSX />}`-style conditional rendering and chase the gate variable — that's pattern (a) setter-to-true verification. Pattern (a) misses dead UI hidden behind static CSS gates: `<div className="...hidden">` or `style={{ display: 'none' }}` on unconditionally-rendered JSX. The element renders, the CSS hides it; no gate variable exists for setter-to-true to find. Pattern (b) CSS-gated verification chases string literals inside `className` attributes for hardcoded `hidden`/`display:none`/`visibility:hidden` without a toggling source. Both must run. Step 7 Commit 9 used (a) to find a dead 370-line zone modal; Commit 12 used (b) to find a dead 80-line "Estimation dynamique" sidebar that (a) couldn't see. See `docs/audit.md` §3 (Cluster C) and the working memory file `dead-ui-detection-patterns` for the full principle.

## Consult the design source-of-truth before deleting UI or inventing UX

For UI/UX-adjacent decisions during refactoring, consult the product's design source-of-truth (Figma, design system docs) before deleting surfaces, inventing UX (loading states, empty states, error states), making structural choices that touch user-visible behavior, or changing copy/labels/button text. The codebase answers *what is*; the design artifact answers *what should be*. They diverge frequently in projects with regression or incomplete-implementation history. Treating the codebase as the source of truth equates "not implemented" with "not intended" — and the two are not the same. By the time the gap is discovered through user feedback or a downstream audit, the deletion is already shipped and restoration is more expensive than the original consult. Step 7 Commit 3 deleted `pages/Perfor.tsx` as "dead code" without consulting Figma; `Performances.png` showed the intended page, and issue #19 had to be opened retroactively. Step 7 formalized the rule from Commit 9 onward; Commit 11 produced its first proactive product-gap finding (issue #21 — post-cart recommendations placement). See `docs/audit.md` §3 (Cluster D) and the working memory file `design-source-of-truth-consultation` for the generalizable principle.

## Extract-without-rewrite when production logic has diverged from a pre-existing pure wrapper

When a pre-existing pure abstraction (hook wrapper, service function, helper) exists but the consumer's inline logic has acquired production behaviors the abstraction doesn't model, the correct discipline for a scoped extraction commit is to extract the inline logic verbatim and track the abstraction-adoption decision as a separate issue (Path B). Adopting + expanding the abstraction to model the divergences (Path A) collapses the commit's safety profile from "decomposition without logic change" to "wide change without test coverage" — exactly the profile that produces regressions in money-adjacent flows. The pure wrappers are attractive ("the work is already half-done") but if production has drifted from their contract, adopting them is a rewrite no matter how it's framed. Audit the divergences explicitly during inventory; extract verbatim; open a tracking issue with the enumerated divergences and 2-3 resolution options; reference the issue in the commit body. Step 7 Commit 11 Save/AddToCart extraction surfaced 5 divergences between the parent's inline handlers and the `useCampaignWizard` Commit-5 wrappers; Path B preserved production behavior and tracked the adoption debt as issue #20. See `docs/audit.md` §3 (Cluster B) and the working memory file `extraction-without-rewrite-discipline`.
