...
_As of commit `5ecd223` (post-TBD-C path-alias sweep; dead-code deletion was TBD-J `78cdfc8`; restructure was Step 8 `6af378e`)._
...
| `pnpm lint`                        | **357 problems (339 errors, 18 warnings)** — Step 8 left 364; TBD-J's deletion removed 6 → 358; TBD-C's alias sweep cleared 1 more (a long-standing `import-x/order` error in NewCampaign.tsx, fixed when the alias-driven reorder moved an interleaved CSS import) → 357. `import-x/order` now 0. **`no-console`: 0** ✓ · **`@typescript-eslint/no-explicit-any`: 0** ✓ · **`@typescript-eslint/no-unused-vars`: 0** ✓ · **`no-empty`: 0** ✓. |
| `pnpm test`                        | 8 suites pass; **98 tests**; 0 failures (unchanged across Step 8) |
| `pnpm --filter @toodooh/web build` | passes — main `index-*.js` gzip **131.47 kB** (flat through TBD-C — the alias sweep is rename-only at the import layer). NewCampaign chunk gzip **26.04 kB** (was 25.76; +0.28 kB minification variance from TBD-C's import reorder, within tolerance). |
