...
| `pnpm typecheck`                   | **51 errors** (held flat across every Step 10 commit — the migration adds only new, fully-typed hook files and touches no Cat-A untyped-root cascade). All remaining are Cat-A pre-existing untyped-root cascades tracked in #15 for Phase 1.                                                                                                                                                                         |
| `pnpm lint`                        | **346 problems (339 errors, 7 warnings)** — TBD-C left 357; Step 10 cleared 11 `react-hooks/exhaustive-deps` warnings (18 → 7) by deleting non-compliant `useEffect` data-fetchers as their reads moved to `useQuery`. Errors flat at 339 throughout. `import-x/order` 0. **`no-console`: 0** ✓ · **`@typescript-eslint/no-explicit-any`: 0** ✓ · **`@typescript-eslint/no-unused-vars`: 0** ✓ · **`no-empty`: 0** ✓. |
| `pnpm test`                        | 21 suites pass; **160 tests**; 0 failures (98 → 160 across Step 10 — query-key factory tests, extracted pure-fn transform tests, the `markFeedRead` optimistic-rollback contract test)                                                                                                                                                                                                                                |
| `pnpm --filter @toodooh/web build` | passes — main `index-*.js` gzip **139.60 kB** (131.47 → 139.60; ~+8 kB one-time `@tanstack/react-query` runtime at Commit 1, flat per-feature-commit thereafter; `@tanstack/react-query-devtools` is dev-only, 0 prod bytes). NewCampaign chunk gzip **26.46 kB** (26.04 baseline + 0.42, within tolerance).                                                                                                          |
