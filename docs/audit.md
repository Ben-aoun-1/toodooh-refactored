...
See `CLAUDE.md` for the phase scope and engineering rules.
...
| `pnpm test`                        | 3 suites pass (22 tests); 1 pre-existing import-time failure (`campaign-hourly-location-plan.service.test.ts` — needs `VITE_SUPABASE_URL`)                                                                                                                                                                                                                                                                                                  |
| `pnpm --filter @toodooh/web build` | passes — 110 chunks, main entry ~434 kB (largest chunk `Dashboard` ~622 kB)                                                                                                                                                                                                                                                                                                                                                                 |
| CI (`main`)                        | **red** — expected; goes green at Step 12                                                                                                                                                                                                                                                                                                                                                                                                   |
...
`MyClients.tsx`, `MyRecharges.tsx`, `UserProfile.tsx` (and `NewCampaign.tsx`, handled by Step 4).
→ **Step 2 · #1**
...
`stores/auth.store.ts` (Zustand, ~20.8 kB) coexists with `services/auth.service.ts` (~1055 lines)
which also reads/writes `localStorage` directly and owns chunks of session/profile logic;
`stores/admin.store.ts` is separate. No React `createContext`, no second `*.store.ts`. CLAUDE.md
mentions "collapsing dual auth stores" but on disk there is only one store — the exact target is
TBD, so Step 3 starts with an investigation. → **Step 3 · #2**
...
449 occurrences of the brand hex; should be a Tailwind `brand` theme token. → **Step 11 · #10**
...
`react-hooks/exhaustive-deps` warnings (latent stale-closure risks). → **Step 10 · #9**

### Flat folder structure

`src/` is flat `pages/` + `components/` + `services/` + `stores/`, not the feature-based
`src/features/<domain>/` the architecture conventions call for. → **Step 5 · #4**

### No server-state layer

TanStack Query is not used; data fetching is hand-rolled (call a service in a component, `setState`).
Conventions want React Query for server state and Zustand for client state, no mixing. → **Step 7 · #6**

### Debug cruft

`App.tsx` has `import './utils/clearAuthCache'; // Utilitaire de debug`; `utils/clearAuthCache.ts`
itself; scattered emoji debug `console.log`s (cleared as part of Step 8). → **Steps 3 & 8 · #2, #7**
...
re-enabled after the structural refactor. → **Step 9 · #8**
...
Ordering principle: **decisions & deletions first** (shrink the surface) → **structure** (so later
mechanical passes run once on the final shape) → **mechanical sweeps last** (console / typing /
a11y / styling / tsconfig). Each step gets its own brainstorm → spec → plan → execute cycle.
...
| 3   | Investigate & consolidate the auth-state layer                      | `stores/auth.store.ts`, `services/auth.service.ts`, `localStorage` users, `App.tsx` | single auth-state layer; no raw `localStorage` outside Zustand `persist`; debug import gone | #2    | ☐      |
| 4   | Decompose `Dashboard.tsx` / `NewCampaign.tsx`                       | `pages/Dashboard.tsx`, `pages/NewCampaign.tsx`, `App.tsx` routes                    | each route renders its own page; largest chunk materially smaller; no regressions           | #3    | ☐      |
| 5   | Restructure `src/` into `src/features/<domain>/`                    | almost all of `src/`                                                                | feature-based layout per conventions; imports updated; build OK                             | #4    | ☐      |
| 6   | Replace `window.location.reload()` in `MyAccount.tsx`               | `pages/MyAccount.tsx`                                                               | 0 `window.location.reload()` calls                                                          | #5    | ☐      |
| 7   | Introduce React Query for server state                              | new query layer; (post-decomposition) pages                                         | server data via React Query; Zustand limited to client state                                | #6    | ☐      |
| 8   | Frontend logger + `console.*` purge                                 | new logger; ~917 call sites; ESLint config                                          | 0 `no-console` errors                                                                       | #7    | ☐      |
| 9   | Typing pass: fix `as any`, reduce tsc baseline, re-tighten tsconfig | many files; `tsconfig.app.json`                                                     | typecheck 0 (or documented residue); 0 `as any`; tsconfig un-softened                       | #8    | ☐      |
| 10  | `jsx-a11y` + `exhaustive-deps` cleanup                              | many `.tsx` files                                                                   | 0 `jsx-a11y/*` errors; 0 `exhaustive-deps` warnings                                         | #9    | ☐      |
| 11  | Tailwind `brand` token: replace hardcoded `#00B3A6`                 | `tailwind.config.js`; 449 call sites                                                | 0 hardcoded `#00B3A6`; `brand` token in use                                                 | #10   | ☐      |
| 12  | Get CI green                                                        | `.github/workflows/ci.yml`                                                          | CI green on `main`; no `--no-verify` needed for normal commits                              | #11   | ☐      |
